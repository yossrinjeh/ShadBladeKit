                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-3">
                            <div class="p-2 {{ $role->name === 'admin' ? 'bg-red-100 dark:bg-red-900/20' : 'bg-blue-100 dark:bg-blue-900/20' }} rounded-lg">
                                <svg class="w-5 h-5 {{ $role->name === 'admin' ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    @if($role->name === 'admin')
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                    @else
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-48 overflow-y-auto">
                        ...
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="rounded border-input">
    ...
    <div id="edit-role-modal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
        <div class="bg-background rounded-lg p-6 w-full max-w-md mx-4">
            ...
            <form id="edit-role-form" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium mb-2">Permissions</label>
                    <div class="space-y-2 max-h-48 overflow-y-auto" id="edit-permissions">
                        @foreach($permissions as $permission)
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="rounded border-input edit-permission" data-permission="{{ $permission->id }}">
                            <span class="text-sm">{{ $permission->name }}</span>
                        </label>
                        @endforeach
                    </div>
